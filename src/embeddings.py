...
def get_hetero_data(
    ...
    node_to_id_dict = {k: v for k, v in nodes_df[["Node", "index"]].values}

    edges_df["index"] = edges_df["source"].apply(lambda node: node_to_id_dict[node])
    edges_df["index_target"] = edges_df["target"].apply(
        lambda node: node_to_id_dict[node]
    )

    merged_df = edges_df

    data["recipe", "has_ingr", "ingr"].edge_index = Tensor(
        get_edge_index_from_label_type(merged_df, "has_ingr")
    ).to(dtype=torch.int64)
    data["ingr", "also_known_as", "ingr"].edge_index = Tensor(
        get_edge_index_from_label_type(merged_df, "also_known_as")
    ).to(dtype=torch.int64)
    data["ingr", "has_sub", "ingr"].edge_index = Tensor(
        get_edge_index_from_label_type(merged_df, "has_sub")
    ).to(dtype=torch.int64)
