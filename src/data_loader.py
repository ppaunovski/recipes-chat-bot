...
def get_nodes_and_egdes_df(
    composed_graph: nx.DiGraph,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    nodes_df = pd.DataFrame({"Node": composed_graph.nodes()})
    ...
    nodes_df["type"] = nodes_df["Node"].apply(
        lambda elem: "recipe" if elem in list_of_recipes_from_df else "ingr"
    )
